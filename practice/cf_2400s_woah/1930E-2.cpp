...
long count_invalid(long n, long k, long window) {
  // stars: 2k-1
  // blanks: n-w
  return choose(n - window + 2*k - 1, 2*k - 1);
}
...
void solve() {
  ...
  for (int k = 1; k <= (n-1)/2; k++) {
    ...
    for (int window_size = 2*k; window_size < n; window_size += 2*k) {
      ...
      ans -= count_invalid(n, k, window_size);
      fprintf(stderr, "  raw choose %ld, subtract %ld\n", choose(n, window_size), count_invalid(n, k, window_size));
      ans += MOD;  ans %= MOD;
    }
    cout << ans << " ";
  }
