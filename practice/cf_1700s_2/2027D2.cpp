...
void update(pairll& state_before, pairll new_state) {
  if (new_state.first < state_before.first) {
    // overwrite
    state_before.first = new_state.first;
    state_before.second = new_state.second;
  } else if (new_state.first == state_before.first) {
    // tie, increase count
    state_before.second += new_state.second;
    state_before.second %= MOD;
  }
