...
void solve() {
  ...
  SegmentTree<int, decltype(max_combiner)> segment_tree(logn, max_combiner, -1, b);

  int max_i = 1;
  while (true) {  //executes at most n times
    int next_max_i = segment_tree.range_query(1, max_i);
    if (next_max_i <= max_i)  break;
    max_i = next_max_i;
  }
