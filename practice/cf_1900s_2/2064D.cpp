...
Node node_empty() {
  return Node{vector<int>(), {-1, -1}};
}


void cull(vector<Node>& trie, int cur, vector<int>& answers, int answer) {
  if (trie[cur].next[0] != -1)  cull(trie, trie[cur].next[0], answers, answer);
  if (trie[cur].next[1] != -1)  cull(trie, trie[cur].next[1], answers, answer);
  for (int id : trie[cur].ids)  answers[id] = answer;
  trie[cur].next[0] = -1;
  trie[cur].next[1] = -1;
}
