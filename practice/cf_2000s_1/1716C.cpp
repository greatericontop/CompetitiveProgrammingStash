...
void solve() {
  ...
  // Top prefixes 1, 3, ...
  {
    vector<long> adjusted_down(m+1);
    FORI1(m) {
      // down[1] has 0 extra, down[2] has 1 extra, etc
      adjusted_down[i] = adown[i] + (i-1);
    }
    long running_max = 0;  for (int j = 1; j <= m; j++)  running_max = max(running_max, adjusted_down[j]);
    for (int i = m; i >= 1; i--) {
      // e.g. atop[m] has +m, atop[m-1] has +m+1
      running_max = max(running_max, atop[i] + m + (m-i));
      if (i % 2 == 1) {
        prefixes[i] = running_max;
      }
    }
  }
  // Bottom prefixes
  {
    vector<long> adjusted_top(m+1);
    FORI1(m) {
      adjusted_top[i] = atop[i] + (i-1);
    }
    long running_max = 0;  for (int j = 1; j <= m; j++)  running_max = max(running_max, adjusted_top[j]);
    for (int i = m; i >= 1; i--) {
      running_max = max(running_max, adown[i] + m + (m-i));
      if (i % 2 == 0) {
        prefixes[i] = running_max;
      }
    }
