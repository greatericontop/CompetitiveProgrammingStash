...
void set_dist(int v, const AdjList& adj, vector<int>& dist) {
  for (int child : adj[v]) {
    dist[child] = dist[v] + 1;
    set_dist(child, adj, dist);
  }
}
struct Entry {
  int v;
...
void solve() {
  ...
  AdjList adj(n+1);
  create_directed_adj(n, adj_undirected, parents, adj);
  vector<int> dist(n+1, -1);
  dist[n] = 0;
  set_dist(n, adj, dist);
  vector<Entry> entries(n);
  FORI1(n)  entries[i-1] = {i, dist[i]};
  sort(entries.begin(), entries.end(), [](const Entry& a, const Entry& b) {
    return a.dist > b.dist;
  });  //farthest first

  bool current_state = (dist[1] % 2);  //true=odd, false=even
  vector<pairii> ops;
  for (const auto entry : entries) {
    if (entry.v == n)  break;
    bool state_of_v = (entry.dist % 2);
    if (state_of_v == current_state) {
      // flip
      ops.pb({1, -1});
