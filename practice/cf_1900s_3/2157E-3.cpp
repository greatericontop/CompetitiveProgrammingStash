...
/* Simulate */
bool sim(int n, int k, int m, const vector<int>& freq, set<int> non_bricks) {
  fprintf(stderr, "called sim with m=%d\n", m);
  vector<int> new_freq = freq;
  if (m == 0)  goto finish;
  for (int x = 3*n; x >= 0; x--) {  //highest first
    int ct = new_freq[x];
    if (ct == 0)  continue;
    ct--;  //one that stays here
    while (ct > 0) {
      auto it = non_bricks.lower_bound(x);
      //assert(it != non_bricks.end());
      int y = *it;
      if (y > x + m) {
