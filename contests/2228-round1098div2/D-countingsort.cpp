...
void solve() {
  ...
  vector<vector<Point>> counting_sort_x(n+1);
  for (int i = 0; i < n; i++) {
    counting_sort_x[points[i].x].pb(points[i]);
  }
  int j = 0;
  for (int i = 0; i <= n; i++) {
    for (const auto& p : counting_sort_x[i]) {
      points[j++] = p;
    }
  }
  ...
  vector<vector<Point>> counting_sort_y(n+1);
  for (int i = 0; i < n; i++) {
    counting_sort_y[points[i].y].pb(points[i]);
  }
  j = 0;
  for (int i = 0; i <= n; i++) {
    for (const auto& p : counting_sort_y[i]) {
      points[j++] = p;
    }
  }
